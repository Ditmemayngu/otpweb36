...
def load_domains():
    if not os.path.exists(DOMAINS_FILE):
        save_domains(["shopthanhhoa.online", "shop36.site"])
...
def generate_two_cross_domain():
    ...
    if len(domains) < 2:
        return None, "Cần ít nhất 2 domain để tạo 2 email xen kẽ không trùng domain."

    selected_domains = random.sample(domains, 2)
    return [{"domain": d, "email": random_email(d)} for d in selected_domains], None
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family: Arial, sans-serif; max-width: 1180px; margin: 24px auto; padding: 0 16px; background: #fafafa; color: #111; }
        input, select, button { padding: 10px; font-size: 15px; }
        button { cursor: pointer; border: 1px solid #bbb; background: white; border-radius: 5px; }
        button:hover { background: #f2f2f2; }
        .box { border: 1px solid #ddd; border-radius: 10px; padding: 16px; margin: 16px 0; background: white; }
        .admin-only { border-color: #c6a700; background: #fffdf0; }
        .error { color: #b00020; font-weight: bold; }
        .ok { color: green; font-weight: bold; }
        .small { color: #666; font-size: 13px; }
        .otp { font-size: 26px; font-weight: bold; color: #d00; }
        .input-row { display: flex; gap: 8px; }
        .input-row input { flex: 1; }
        .generated-table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        .generated-table th, .generated-table td { border: 1px solid #ddd; padding: 10px; }
        .generated-table th { background: #f1f1f1; }
        .generated-email { font-weight: bold; }
        .notice { border: 1px solid #d8d8d8; background: white; padding: 14px; border-radius: 10px; margin: 14px 0; }

        .inline-inbox-section { margin-top: 18px; }
        .inbox-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; align-items: start; }
        .mailbox-card { border: 1px solid #d8d8d8; border-radius: 10px; background: #fff; overflow: hidden; }
        .mailbox-top { text-align: center; padding: 16px; border-bottom: 1px solid #ddd; background: #fbfbfb; }
        .email-line { display: flex; justify-content: center; align-items: center; gap: 0; margin: 8px 0; }
        .email-box { border: 1px solid #ccc; padding: 10px; min-width: 300px; font-weight: bold; background: white; overflow-wrap: anywhere; }
        .refresh-line { display: flex; justify-content: center; gap: 10px; align-items: center; margin-top: 8px; }

        .inbox-wrap { display: grid; grid-template-columns: 38% 62%; min-height: 330px; }
        .inbox-list { border-right: 1px solid #ddd; max-height: 420px; overflow-y: auto; }
        .inbox-title { padding: 10px; border-bottom: 1px solid #ddd; font-size: 18px; background: #fff; position: sticky; top: 0; }
        .message-item { display: block; padding: 10px; border-bottom: 1px solid #eee; color: #111; text-decoration: none; }
        .message-item:hover { background: #f5f5f5; }
        .message-item.active { background: #eef5ff; }
        .subject { font-weight: bold; font-size: 15px; margin-bottom: 4px; }
        .from { color: #0066cc; font-size: 13px; margin-bottom: 4px; overflow-wrap: anywhere; }
        .snippet { color: #555; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .message-view { padding: 12px; max-height: 420px; overflow-y: auto; }
        .message-head { border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 10px; }
        .message-head h3 { margin-top: 0; }
        .body-box { border: 1px solid #ddd; padding: 10px; min-height: 150px; white-space: pre-wrap; overflow-wrap: anywhere; background: #fff; font-size: 13px; }
        @media (max-width: 980px) {
            .inbox-grid-2 { grid-template-columns: 1fr; }
        }
        @media (max-width: 700px) {
            .inbox-wrap { grid-template-columns: 1fr; }
            .inbox-list { border-right: 0; border-bottom: 1px solid #ddd; }
        }
    </style>
</head>
<body>
    ...
    <div class="box">
        ...
        <form method="GET" action="/inbox" class="input-row">
            <input type="text" name="email" placeholder="Ví dụ: jj6q10kn@shop36.site" value="{{ inbox_email or '' }}" required>
    ...
    <div class="box admin-only">
        ...
        {% if admin %}
            ...
            <p class="small">Chỉ admin mới dùng được. Mỗi lần tạo lấy 2 domain khác nhau, không trùng nhau.</p>

            <form method="POST" action="/generate">
                <button type="submit">Tạo 2 email random</button>
            </form>
            ...
            {% if generated_emails %}
                <div class="notice">
                    ...
                    <table class="generated-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><button onclick="navigator.clipboard.writeText('{{ item.email }}')">Copy</button></td>
                            </tr>
                            {% endfor %}
                        </tbody>
                    </table>
                    ...
                    <div class="inline-inbox-section">
                        <h4>📬 Inbox tự hiện cho 2 email vừa tạo</h4>
                        <div class="inbox-grid-2">
...
<div class="mailbox-card">
    <div class="mailbox-top">
        ...
        <div class="email-line">
            ...
            <button onclick="navigator.clipboard.writeText('{{ inbox.email }}')">📋 Copy</button>
        </div>
        <div class="refresh-line">
            <a href="{{ url_for('inbox', email=inbox.email) }}"><button>⟳ Refresh riêng</button></a>
        </div>
        {% if inbox.error %}
    ...
    <div class="inbox-wrap">
        <div class="inbox-list">
            <div class="inbox-title">Inbox <span class="small">({{ inbox.count }})</span></div>
            ...
            {% if inbox.selected_message %}
                <div class="message-head">
                    ...
                    {% if inbox.selected_message._otp %}
                        ...
                        <button onclick="navigator.clipboard.writeText('{{ inbox.selected_message._otp }}')">Copy OTP</button>
                    {% endif %}
                </div>
...
def render(**kwargs):
    ...
    if generated_emails:
        generated_inboxes = [build_inbox_data(item["email"]) for item in generated_emails]
